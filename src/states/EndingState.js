...
class EndingScene extends Phaser.State {
  ...
  create () {
    ...
    scoreText = this.game.add.bitmapText((logicWorldBounds.width / 2) - 16, logicWorldBounds.height / 2 - this.game.cache.getImage('restart').height, 'nokia16', `SCORE: ${this.finalScore || '00000'}`, 16);
    restart = this.game.add.button((logicWorldBounds.width / 2) - this.game.cache.getImage('restart').width, logicWorldBounds.height / 2 - this.game.cache.getImage('restart').height, 'restart', this.restartGame, this, 2, 1, 0);
